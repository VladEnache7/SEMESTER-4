...
class MoviesRepo:
    ...
    # db = Annotated[models.Movie, Depends(get_database)]
    ...
    def generate_and_add_movies(db: db_dependency_movies, count):
        ...
        movie_names = [movie['name'] for movie in MoviesRepo().get_all_movies()]
        movies = []
        for i in range(2, count + 2):
            ...
            while True:
                ...
                if new_movie_name not in [movie['name'] for movie in MoviesRepo().get_all_movies()]:
                    break
    ...
    def generate_and_add_characters(db: db_dependency_characters, count):
        ...
        character_names = [character['name'] for character in MoviesRepo().get_all_characters(db)]
        characters = []
        for i in range(2, count + 2):
            # generate a random character name ensuring that the name is unique
            while True:
                new_character_name = random.choice(character_names)
                # if the new_character_name does not have a number at the end, add 2 at the end
                if not new_character_name[-1].isdigit():
                    new_character_name += " 2"
                else:
                    # if the new_character_name has a number at the end, increment the number by 1
                    new_character_name = new_character_name[:-1] + str(int(new_character_name[-1]) + 1)

                if new_character_name not in [character['name'] for character in MoviesRepo().get_all_characters(db)]:
                    break

            character = CharacterBase(name=new_character_name,
                                      movie_name=random.choice(
                                          [movie['name'] for movie in MoviesRepo().get_all_characters(db)]),
                                      description=f"Generated Description for character {new_character_name} at {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}")

            MoviesRepo().add_character(db, character)
