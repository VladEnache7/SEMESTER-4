...
class MoviesRepo:
    ...
    def add_movies(self, movies):
        ...
        for movie in movies:
            ...
            self.highest_id += 1
            self.movies.append(movie_dict)
            added_movies.append(movie_dict)
        return added_movies
    ...
    def generate_and_add_movies(self, count):
        ...
        for i in range(2, count + 2):
            # movie = {
            #     "name": f"{random.choice(movie_names)} {i}",
            #     "year": random.randint(1950, 2022),
            #     "duration": f"{random.randint(1, 3)}h {random.randint(0, 59)}m",
            #     "genre": random.choice(["Animation", "Adventure", "Comedy", "Action", "Drama", "Family", "Fantasy"]),
            #     "description": "Test Description"
            # }
            movie = MovieBase(name=f"{random.choice(movie_names)} {i}",
                              year=random.randint(1950, 2022),
                              duration=f"{random.randint(1, 3)}h {random.randint(0, 59)}m",
                              genre=random.choice(
                                  ["Animation", "Adventure", "Comedy", "Action", "Drama", "Family", "Fantasy"]),
                              description=f"Test Description - {count} generated movies")

            self.add_movie(movie)
            movies.append(movie)
        return movies

    def generate_and_add_movies_periodically(self, count):
        self.generate_and_add_movies(count)
        self.generation_count += 1
        if self.generation_count < 20:
            threading.Timer(1, self.generate_and_add_movies_periodically, args=[count]).start()
            print(f"Generating {self.generation_count} time movies in background every 1 second")
